...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    if (role === "freelancer") {
      req1 = axios.get(
        `http://localhost:3000/freeMS/msg/${id}/${index}/${jobbi}`
      );
      req2 = axios.get(
        `http://localhost:3000/jobMS/msg/${index}/${id}/${jobbi}`
      );
      ...
      req2 = axios.get(
        `http://localhost:3000/freeMS/msg/${index}/${id}/${jobbi}`
      );
      req1 = axios.get(
        `http://localhost:3000/jobMS/msg/${id}/${index}/${jobbi}`
      );
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3000/job/jb/" + jobbi)
      .then((res) => {
        setJob(res.data);
      })
      .catch((err) => {
  ...
  const handleRoute = (freelancerId: number, jobownerId: number) => {
    ...
    } else route.push(`/chat/${freelancerId}?id=${jobbi}`);
  };
  ...
  function HandleDate(dateTimeString: string) {
    ...
    const formattedDateTime = ` ${formattedHours}:${
      minutes < 10 ? "0" : ""
    }${minutes} ${period},${day} ${month}`;
  ...
  function HandleDateMe(dateTimeString: string) {
    ...
    const formattedDateTime = ` ${day} ${month},${formattedHours}:${
      minutes < 10 ? "0" : ""
    }${minutes} ${period}`;
  ...
  const handleSend = () => {
    if (role === "freelancer") {
      axios
        .post("http://localhost:3000/freeMS", {
          sender: id,
          reciever: index,
          body: currentMessage,
          idjob: jobbi,
        ...
        .then(() => {
          console.log("the freelancer sended very cute message");
          sendMessage({
            sender: id,
            reciever: index,
            body: currentMessage,
      ...
      axios
        .post("http://localhost:3000/jobMS", {
          sender: id,
          reciever: index,
          body: currentMessage,
          idjob: jobbi,
        ...
        .then(() => {
          console.log("the jobowner sended very cute message");
          sendMessage({
            sender: id,
            reciever: index,
            body: currentMessage,
        ...
        <div className="flex flex-row justify-between bg-white">
            ...
            <div className="flex flex-col mt-5 overflow-y-auto max-h-[481px]">
              {myMessage.map((el, i) => (
                <div
                  ...
                  className={
                    handleClassName(
                      el[el.length - 1].freelancer.id,
                      el[el.length - 1].jobowner.id
                    ) + " flex flex-row py-4 px-2 cursor-pointer"
                  }
                  onClick={() => {
                    handleRoute(
                      el[el.length - 1].freelancer.id,
                ...
                >
                  <div className="w-1/4">
                    <img
                      src={
                        role === "freelancer"
                          ? el[el.length - 1].jobowner.image
                          : el[el.length - 1].freelancer.image
                      }
                  ...
                  <div className="w-full">
                    <div className="text-lg font-semibold">
                      {role === "freelancer"
                        ...
                        : el[el.length - 1].freelancer.name}
                    </div>
          ...
          <div className="w-full px-5 flex flex-col justify-between">
                ...
                <div key={i}>
                  ...
                  {message.him ? (
                    <div className="flex justify-start mb-4">
                      ...
                      <span className="text-sm ml-[10px] font-normal text-gray-500 dark:text-gray-400">
                        {HandleDate(message.createdAt)}
                      </span>
                    </div>
                  ) : (
                    <div className="flex justify-end mb-4">
                      <span className="text-sm mr-[10px] font-normal text-gray-500 dark:text-gray-400">
                        {HandleDateMe(message.createdAt)}
                      </span>
            ...
            <div className="py-5">
              ...
              />
              <button
                className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-3 px-4 border border-blue-700 rounded-xl"
                onClick={() => {
                  handleSend();
                }}
              >
          ...
          <div className="w-2/5 border-l-2 px-5">
            <div className="flex flex-col">
              ...
              <div className="font-light mb-[20px]">
                {role === "freelancer"
                  ? `Welcome ${chatOwner?.name}! We're thrilled to have you join our community of talented freelancers. Here, you'll find a wide range of exciting job opportunities that match your skills and expertise.`
                  : ` We're thrilled to have you on board ${chatOwner?.name}! Your journey to finding the perfect freelancers begins here.
            Our platform is designed to connect you with skilled professionals who can bring your projects to life.`}
                <div>
                  {role === "freelancer" ? (
                    ...
                    <Link
                      href={`/createReview/${chatReciever?.id}?id=${chatOwner?.id}`}
                    >
                      <button
                        type="button"
                        data-te-ripple-init
                        data-te-ripple-color="light"
                        className="mb-4  mt-4 flex rounded-3xl px-6 pb-2 pt-2.5 text-s font-bold leading-normal gap-1 text-black hover:text-[#267296] border-2"
                      >
                        <svg
                        ...
                        Add A review For {`${chatReciever?.name}`}
                      </button>
                    </Link>
                  )}
                </div>
