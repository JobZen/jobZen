...
import Nav from '@/app/navjobowner/page';
...
function page() {
    const[freebycateg,setFreebycateg]=useState<Freelancer[]>([])
    ...
    useEffect(() => {
        ...
        axios
        .get(`http://localhost:3000/freelancer/freebycat/${index}`)
        .then((res) => {
          const allFreelancer: Freelancer[] = res.data;
          setFreebycateg(allFreelancer);
        })
        .catch((err) => {
          console.log(err);
        });
